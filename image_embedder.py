...
preprocess = models.ResNet18_Weights.IMAGENET1K_V1.transforms() # Use the last version of the transforms
...
class ImageEmbedder:
    ...
    def compute_color_histograms(self):
        ...
        for i in tqdm(range(0, len(self.df_det), batch_size)):
            ...
            bboxes = self.df_det[["bb_left", "bb_top", "bb_width", "bb_height"]].values[i:min(i + batch_size, len(self.df_det))]

            # make histogram for the given bbox
            for el in [self.make_histogram(bbox, frame) for bbox, frame in zip(bboxes, self.df_det.frame.values[i:min(i + batch_size, len(self.df_det))])]:
    ...
    def compute_embeddings(self):
        ...
        for i in tqdm(range(0, len(self.df_det), batch_size)):
            ...
            images = [self.frames[i - 1] for i in self.df_det.frame.values[i:min(i + batch_size, len(self.df_det))]]

            # Crop all images
            cropped = [img[int(y) : int(y + h), int(x) : int(x + w)] for (x, y, w, h), img in zip(bboxes, images)]
    ...
    def compute_cosine_similarity(self, embedding1, embedding2):
        return torch.abs(torch.nn.CosineSimilarity(dim=0)(embedding1, embedding2)).item()
